...
class MultiInfer:
    ...
    def handle(self):
        ...
        for area in self.areas:
            ...
            #weighted weather
            province_original_copy = df[df["station"] == area].copy().reset_index()
            if province_original_copy.shape[0]!=1:
                self.logger.indo(f"有不止一行的station是{area}")
            else:
                value_cols = [f"ssrd_pos_{i+1}_predict"] + ["GHI_SOLARGIS_predict"]
                weighted_key = area
                source = df.loc[df["station"].ne(weighted_key)]
                weighted_values = {}
                for timestamp_win, group in source.groupby("timestamp_win", sort=False):
                    weights = group["cap_power_on"].map(lambda x: x[0]).to_numpy(dtype = float)
                    invalid_mask = ~np.isfinite(weights)
                    if invalid_mask.any():
                        invalid_stations = df.loc[invalid_mask, "station"].tolist()
                        valid_mean = weights[~invalid_mask].mean()
                        weights[invalid_mask] = valid_mean
                        self.logger.info(f"异常站点：", invalid_stations)
                    try:
                        ...
                        temp_dict = {}
                        for col in value_cols:
                            temp_dict[col] = province_original_copy.iloc[0][col]
                        weighted_values[timestamp_win] = temp_dict
                mask = df["station"].eq(weighted_key)
                for col in value_cols:
                    df.loc[mask, col] = pd.Series(
                        [
                            weighted_values[ts][col]
                            for ts in df.loc[mask, "timestamp_win"]
                        ],
                        index = df.index[mask],
                        dtype = object,
                    )
            df = pd.concat([df, province_original_copy], ignore_index = True)
            ...
            # 提取 province 行（station == area），只存在于 normal 中
            df_province = pd.DataFrame()
            if not df_normal_part.empty and area in df_normal_part['station'].values:
                df_province = df_normal_part[df_normal_part['station'] == area].copy()
                df_normal_part.drop(df_normal_part[df_normal_part['station'] == area].index, inplace=True)
                self.logger.info(f"提取 province 行: {area}, shape={df_province.shape}")
            else:
            ...
            # 省级数据-MoE v3.1 入口
            if df_province.shape[0] == 1:
                df_province = pd.concat([df_province] * 2, ignore_index=True)
                df_province.loc[1, "station"] = "unknow_province"
            if df_province.shape[0] > 0:
                self.logger.info(f"省级推理开始, checkpoint: {self.province_checkpoints_dir}")
                config_province = copy.deepcopy(self.config_dict)
                config_province["data_check"]["future_len"] = self.predict_seq_lens
                result_df_province = multi_station_inference(
